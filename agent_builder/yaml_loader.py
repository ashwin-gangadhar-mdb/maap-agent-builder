...
def parse_response_model(response_dict: dict) -> BaseModel:
    ...
    """
    response_dict = list(response_dict.items()) | Map(lambda item: (item[0], tuple(item[1]))) | Pipe(dict)
        ...
        for tool in tools_config:
            if "embedding_model" in tool and isinstance(tool["embedding_model"], str):
                if tool["embedding_model"] not in result.get("embeddings", {}):
                    logger.error(f"Referenced embedding model '{tool['embedding_model']}' not found")
                    raise ValueError(f"Referenced embedding model '{tool['embedding_model']}' not found")
