...
AGENT_CONFIG = {
    ...
    "tool_call": {
        ...
        "required_fields": ["llm"],
        "optional_fields": ["tools", "system_prompt", "system_prompt_path", "checkpointer_config"],
    ...
    "reflect": {
        ...
        "optional_fields": ["tools", "checkpointer_config", "system_prompt_path", "reflection_prompt_path"],
...
def load_agent(config: AgentConfig) -> Any:
    ...
    if agent_type not in AGENT_CONFIG:
        ...
        logger.error(f"Unsupported agent type: {agent_type}. Available types: {available_types}")
    ...
    def load_prompt_from_file(prompt, path, prompt_type="system"):
        ...
        if not prompt and path:
            try:
                ...
                logger.error(f"Failed to load {prompt_type} prompt from {path}: {str(e)}")
                raise ValueError(f"Failed to load {prompt_type} prompt from {path}: {str(e)}")
        return prompt
    ...
    config.reflection_prompt = load_prompt_from_file(
        ...
        config.reflection_prompt_path, 
        "reflection"
    )
    
    # Verify required fields
    missing_fields = [field for field in agent_info["required_fields"] 
    ...
    if config.checkpointer_config:
        ...
        try:
            ...
            logger.warning(f"Failed to create checkpointer, using in-memory default: {str(e)}")
    ...
    if agent_type in ["react", "tool_call"]:
        ...
        if config.additional_kwargs and "reflection_prompt" in config.additional_kwargs:
            ...
            logger.error("Reflection agent requires a reflection_prompt in additional_kwargs")
            raise ValueError("Reflection agent requires a reflection_prompt in additional_kwargs")
